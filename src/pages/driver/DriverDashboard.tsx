...
import { Star, Truck, CheckCircle, Navigation, Loader2 } from 'lucide-react';
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-blue-600" size={40} /></div> : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8">
                ...
                <p className="text-4xl font-black text-slate-900 mt-2">{stats.activeLoads}</p>
            </Card>
            ...
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8">
                ...
                <p className="text-4xl font-black text-slate-900 mt-2">{stats.completedTrips}</p>
            </Card>
        ...
        <div className="bg-slate-900 rounded-[3rem] p-12 text-white text-center shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-slate-400 font-bold max-w-md mx-auto mb-8">سوق الحمولات يحتوي على أكثر من 50 طلب نقل جديد في منطقتك الحالية.</p>
            <Button onClick={() => window.location.href='/driver/loads'} className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl h-14 px-10 font-black text-lg shadow-xl shadow-blue-500/20 transition-all active:scale-95">
                تصفح سوق الحمولات
            </Button>
        </div>
      </div>
